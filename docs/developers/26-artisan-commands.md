...
## admin-core:translations-split (v1.13)

```bash
php artisan admin-core:translations-split [--dry-run] [--resource=all|<имя>]
```

Создаёт недостающие per-locale строки переводов у всех ресурсов,
зарегистрированных через `AdminCore::resource()` с `translatable`-полями.

**Зачем.** Пока у записи нет строк переводов, все локали читают одну базовую
колонку (fallback трейта Translatable). ПЕРВОЕ сохранение через админ-форму
молча «размножает» значения вкладок в независимые копии — редактор, правивший
одну вкладку и проверяющий сайт на другом языке, решает «правка не применилась».
Команда делает это разделение явно и предсказуемо: каждая локаль получает
строку со своим ТЕКУЩИМ эффективным значением (locale → kk → ru → база),
визуально ничего не меняется.

Идемпотентна: существующие непустые переводы не трогает, повторный запуск
дозаполняет только недостающее. Soft-deleted записи тоже обрабатываются
(restore иначе вернул бы сюрприз первого сохранения).

## admin-core:content-cleanup-editor (v1.13)

```bash
php artisan admin-core:content-cleanup-editor [--dry-run] [--target=all|<table>]
```

Узкая чистка артефактов визуального редактора (TipTap): пустые
`<p></p>`/`<p>&nbsp;</p>`/`<p><br></p>` и обёртки `<li><p>…</p></li>`
вокруг одноабзацных пунктов списка. Стили и span'ы не трогает (в отличие от
`content-cleanup-gdocs`). Свипает `admin-core.editor_hygiene.targets` —
см. [editor-hygiene.md](../editor-hygiene.md).
