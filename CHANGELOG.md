...
### Added
- **Редактор блоков показывает ключи `data`, которых нет в схеме типа.** Раньше
  `Form.vue` при наличии схемы прятал сырой JSON, поэтому всё, чего схема не
  объявила, было не видно в админке и правилось только через `tinker`. На ETU
  таких ключей набралось под сотню (`badge`, `stats`, `benefits`, `jobs`,
  `button1_text`, …) — блок открывался с пустыми чужими полями, а реальный
  контент оставался невидимым. Теперь `BlockDataEditor` достраивает виджет по
  форме самого значения: строка → text/textarea, число → number, булево →
  чекбокс, `{ru,kk,en}` → translatable, массив однородных записей → обычный
  репитер (подполя собираются по всем строкам, чтобы `null` в первой не
  обеднил набор), всё остальное → JSON-поле. Такие поля помечены плашкой
  «вне схемы». Схема по-прежнему главная: она задаёт порядок, подписи и
  специальные виджеты, достраивается только хвост.
- **Аварийный «Показать JSON целиком»** в свёрнутом виде рядом с визуальным
  редактором — на случай формы данных, которую не разложить на виджеты.
- Виджеты `checkbox` и `json` в `BlockDataEditor`; `select` на верхнем уровне
  схемы больше не рендерится обычным текстовым полем (варианты игнорировались).

### Fixed
- **Виджет подстраивается под форму значения, а не только под схему.** Данные
  копились годами, и там, где схема объявляет `text`, в блоке мог лежать
  `{ru,kk,en}`: поле показывало `[object Object]`, а сохранение схлопывало
  перевод в строку одной локали. На ETU так было в 19 определениях полей
  (`features → items[].title`, `team → members[].name`, `hero → buttons[].url`,
  `faq → items[].question` …). Теперь `text`/`textarea`/`file` с картой внутри
  автоматически становятся `translatable`/`translatable_textarea`/
  `translatable_file`; колонка репитера переводится, если карта встретилась
  хоть в одной записи; поле, объявленное массивом, но хранящее объект,
  отдаётся достроенному виджету.

Проверка: настоящий `BlockDataEditor` отрендерен SSR-ом на всех 292 блоках ETU
с их реальными схемами — каждый ключ `data` получил виджет, `[object Object]`
не встречается ни разу (до правки — 130 блоков с невидимыми полями и 13 с
`[object Object]`).
